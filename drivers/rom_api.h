...
// functions table int32_t result;                                                     // Declare an int variable result
// = pROMDiv->sidiv(-99, 6);                                    // Call the sidiv routine, result now contains -99/6 =
// -16 ROM_DIV_API_T const *pPwr = LPC_ROM_API->pPWRD;                     // Create and initialize a pointer to the
// power API functions table pPwr->set_power((uint32_t *)&cmd_table, (uint32_t *)&result_table); // Call the set_power
// routine
